...
def is_valid_json_file(file_path: str) -> bool:
    try:
        ...
        with open(file_path, "r", encoding="utf-8") as file:
            ...
            )  # является ли списком, список не пустой, все элементы списка являются словарями
    except:
        return False
...
def json_to_csv(json_path: str, csv_path: str) -> None:
    if not is_valid_json_file(
        json_path
    ):  # JSON файл не соотвествует критериям, выводит ошибку и завершит программу
    ...
    ):  # проверка расширения файлов, lower-метод преобразования в нижний регистр
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=json_data[0].keys()
        )  # создает CSV файл с заголовками из ключей первого элемента JSON, названия колонок = ключи первого элемента
        writer.writeheader()
...
csv_to_json(
    r"C:\Users\lezgi\Desktop\python_labs\data\samples\people.csv",
    r"C:\Users\lezgi\Desktop\python_labs\data\out\people_from_json.json",
)

json_to_csv(
    r"C:\Users\lezgi\Desktop\python_labs\data\samples\people.json",
    r"C:\Users\lezgi\Desktop\python_labs\data\out\people_from_csv.csv",
)
