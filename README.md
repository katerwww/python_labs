...
```
![02](./images/lab08/serialize.png)
![03](./images/lab08/input.png)
![04](./images/lab08/output.png)

# ___Лабораторная работа 9___
## group
...
```
![01](./images/lab09/group.png)
## до удаления
![02](./images/lab09/было.png)
## после
![03](./images/lab09/remove.png)
![04](./images/lab09/стало.png)

# ___Лабораторная работа 10___
## Теория 
### **Стек (Stack) — структура с принципом LIFO**
**Основная идея:** Элементы добавляются и извлекаются с одного конца (вершины). Последний добавленный элемент будет первым удалённым.
...
- **Все операции работают за O(1)** при правильной реализации
- **Время выполнения** постоянно, так как операции затрагивают только один конец структуры

**Типичные сценарии использования:**
- **Системы отмены действий** (undo/redo) — каждое действие помещается в стек
- **Обход деревьев и графов** в глубину (DFS) — стек хранит узлы для посещения
- **Синтаксический анализ** — проверка корректности скобок, преобразование выражений
- **Управление вызовами функций** — стек вызовов в большинстве языков программирования


---

### **Очередь (Queue) — структура с принципом FIFO**
**Основная идея:** Элементы добавляются в конец и извлекаются из начала. Первый добавленный элемент будет первым удалённым.

**Ключевые характеристики:**
- **Три основные операции:**
  - `enqueue(x)` или `put(x)` — добавление элемента в конец
  - `dequeue()` или `get()` — удаление и возврат элемента из начала
  - `front()/peek()` — просмотр первого элемента без удаления
- **Все операции работают за O(1)** при правильной реализации

**Особенности в Python:**
- **`list` не подходит для очереди** — `pop(0)` имеет сложность O(n) из-за сдвига всех элементов
- **`collections.deque` — оптимальный выбор** — двусторонняя очередь с O(1) операциями с обоих концов

---

### **Связные списки: принцип и разновидности**

#### **Односвязный список (Singly Linked List)**
**Структурная организация:**
- Состоит из узлов, каждый содержит:
  - `data` — полезные данные
  - `next` — указатель на следующий узел или `None` для последнего
- Элементы **не расположены последовательно** в памяти (в отличие от массивов)

**Преимущества:**
- **Быстрые операции в начале списка:**
  - Добавление (`prepend`) — O(1)
  - Удаление из начала — O(1)
- **Эффективное удаление из середины** (при наличии ссылки на узел) — O(1)
- **Динамический размер** — не требует предварительного выделения памяти
- **Фундамент для других структур** — стеков, очередей, хеш-таблиц

**Недостатки:**
- **Доступ по индексу — O(n)** — требуется последовательный проход от головы
- **Отсутствие обратных ссылок** — сложность удаления узла без знания предыдущего
- **Дополнительная память** на хранение указателей

**Оптимизации:**
- Хранение указателя на хвост (`tail`) позволяет добавлять в конец за O(1)
- Кольцевая организация для циклических структур
...
**Ключевые преимущества перед односвязным:**
- **Удаление по прямой ссылке — O(1)** — не требуется поиск предыдущего узла
- **Двунаправленный обход** — движение как вперёд, так и назад
- **Эффективная работа с обоими концами** — одинаково быстрые операции в начале и конце

**Типичные применения:**
- **LRU-кэши** — быстрое перемещение элементов в начало/конец
- **Навигация в обоих направлениях** — история браузера, плейлисты
- **Сложные структуры** — деки, двусвязные кольцевые списки

**Компромиссы:**
- **Увеличенное потребление памяти** — на 33-50% больше, чем у односвязного
- **Сложность реализации** — больше операций при вставке/удалении
- **Риск ошибок** — необходимо аккуратно обновлять все ссылки
